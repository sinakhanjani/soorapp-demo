...
const getAdminPage = (req, res) => {
  Admin.find({}, (error, foundAdmins) => {
    if (error) {
      req.flash('error', 'خطا در دریافت اطلاعات از سرور');
      return res.redirect('/web/');
    }
    return res.render('./admin/index', { admins: foundAdmins });
  });
};

const getEditPage = (req, res) => {
  Admin.findOne({ _id: req.params.adminID }, (error, foundAdmin) => {
    if (error || foundAdmin === null) {
      req.flash('error', 'خطا در دریافت اطلاعات');
      return res.redirect('/web');
    }
    return res.render('./admin/edit', { admin: foundAdmin });
  });
};
...
const postUpdate = (req, res) => {
  ...
  Admin.findOneAndUpdate({ _id: req.params.adminID }, updateAdminObj, (error, foundAdmin) => {
    if (error || foundAdmin === null) {
      req.flash('error', 'خطا در دریافت اطلاعات');
      return res.redirect('/web/');
    }
    req.flash('success', 'تغییرات با موفقیت ذخیره شد.');
    return res.redirect(`/web/admin/${req.params.adminID}`);
  });
};
